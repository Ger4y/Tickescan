...
const History: React.FC<HistoryProps> = ({ data, onBack, onClear, onDeleteOne }) => {
  ...
  const downloadCSV = () => {
    ...
    data.forEach(row => {
      ...
      let rowData = [`"${safe(row.date)}"`, `"${safe(row.establishment)}"`, `"${safe(row.amount)}"`, `"${safe(row.payer)}"`];
      ...
      if (hasItems) {
        const itemsStr = row.items ? row.items.map(i => `- ${i.description}: ${i.price}`).join("\n") : "";
    ...
    let totalRow = ["", `"${totalLabel}"`, `"${totalValue}"`, ""];
  ...
  return (
    <div className="flex flex-col h-full animate-fade-in bg-gray-50">
      <div className="bg-white p-4 shadow-sm z-10 border-b border-gray-100 flex justify-between items-center shrink-0">
        <div><h2 className="text-xl font-bold text-gray-900">📋 {t.historyTitle}</h2><p className="text-xs text-gray-500">{data.length} {t.ticketsSaved}</p></div>
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-3">
        {data.length === 0 ? (
          <div className="text-center py-10 text-gray-400"><div className="text-4xl mb-3">📭</div><p>{t.noTickets}</p><button onClick={onBack} className="mt-4 text-indigo-600 font-bold hover:underline">{t.scanFirst}</button></div>
        ) : (
          data.slice().reverse().map((item) => (
            <div key={item.id} className="bg-white p-4 rounded-xl shadow-sm border border-gray-200 relative group overflow-hidden">
              ...
              {item.notes && <div className="mt-2 text-xs text-gray-500 bg-gray-50 p-2 rounded border border-gray-100 italic">📝 {item.notes}</div>}
              {onDeleteOne && (
                <button type="button" onClick={(e) => { e.preventDefault(); e.stopPropagation(); handleDeleteItem(item.id); }} className="absolute top-0 right-0 p-3 text-red-400 hover:text-red-600 hover:bg-red-50 transition-colors z-20 cursor-pointer">
                  <svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M6 18L18 6M6 6l12 12" /></svg>
                </button>
              )}
            </div>
      ...
      {data.length > 0 && (
        <div className="bg-white p-4 border-t border-gray-200 shadow-[0_-4px_6px_-1px_rgba(0,0,0,0.05)] shrink-0 z-20">
          <div className="flex justify-between items-center mb-4 px-1"><span className="text-gray-700 font-bold">{t.totalEstimated}</span><span className="text-2xl font-black text-gray-900">{totalAmount.toFixed(2)}</span></div>
          <div className="space-y-3">
            <button onClick={downloadCSV} className="w-full flex items-center justify-center gap-2 bg-green-600 text-white py-3 rounded-xl font-bold hover:bg-green-700 transition shadow-lg shadow-green-200 active:scale-98"><span>📊</span> {t.exportCSV}</button>
            <button onClick={handleClear} className="w-full text-red-600 font-semibold text-sm py-3 hover:bg-red-50 rounded-xl transition border border-transparent hover:border-red-100 active:bg-red-100">{t.clearHistory}</button>
          </div>
        </div>
      )}
    </div>
