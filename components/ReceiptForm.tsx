...
// Internal Keys must match keys in translation file
const CATEGORIES_KEYS = [
  "Supermercado", "Hogar", "Gasolina", "Ocio", 
  "Regalos", "Restaurantes", "Personal", "Viajes"
] as const;

// Helper to handle both dot and comma decimals safely
...
const formatValue = (num: number): string => {
  // Try to keep user's locale style roughly, but standardizing on 2 decimals
  // We'll use dot for storage/input usually unless user typed comma
  return num.toFixed(2); // e.g. "10.50"
};
...
const ReceiptForm: React.FC<ReceiptFormProps> = ({ initialData, settings, imagePreview, onSubmit, onCancel, isSubmitting }) => {
  ...
  const handleItemChange = (index: number, field: keyof ReceiptItem, value: string) => {
    ...
    // Logic: If price changes, update the total amount by the difference
    if (field === 'price') {
      ...
      // Update form data with new item value AND new total
      ...
      setFormData(prev => ({ 
        ...prev, 
        items: newItems,
        amount: newTotal > 0 ? formatValue(newTotal) : "0.00"
      }));
    } else {
  ...
  const addItem = () => {
    // Add item with 0 price initially so it doesn't affect total until edited
    setFormData(prev => ({
      ...prev,
  ...
  // Determine if we should show the extra fields section based on SETTINGS
  const hasExtraFields = settings.extractItemCount || settings.extractCategory;
  
  // Check if items list should be shown (either setting enabled OR data exists)
  ...
  return (
    <div className="flex flex-col h-full animate-fade-in bg-gray-50">
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-5">
        ...
        {/* Preview Image (Collapsible or Small) */}
        {imagePreview && (
          <div className="flex justify-center mb-2">
            <img 
              src={imagePreview} 
              alt="Receipt" 
              className="h-32 object-contain rounded-lg border border-gray-200 bg-white"
            />
          </div>
        )}
        ...
        <form id="receipt-form" onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="grid grid-cols-2 gap-3">
             <div className="space-y-1">
              ...
              <input
                ...
                className="w-full px-3 py-2 rounded-lg border border-gray-300 focus:ring-2 focus:ring-indigo-500 bg-white text-gray-900 placeholder-gray-400"
              />
            </div>
            <div className="space-y-1">
              ...
              <input
                ...
                className="w-full px-3 py-2 rounded-lg border border-gray-300 focus:ring-2 focus:ring-indigo-500 font-mono font-bold text-gray-900 bg-white placeholder-gray-400"
              />
            </div>
          </div>
          ...
          <div className="space-y-1">
            ...
            <input
              ...
              onChange={handleChange}
              placeholder={t.placeholders.est}
              className="w-full px-3 py-2 rounded-lg border border-gray-300 focus:ring-2 focus:ring-indigo-500 bg-white text-gray-900 placeholder-gray-400"
            />
          </div>
          ...
          <div className="space-y-1">
            ...
            <input
              ...
              onChange={handleChange}
              placeholder={t.placeholders.payer}
              className="w-full px-3 py-2 rounded-lg border border-gray-300 focus:ring-2 focus:ring-indigo-500 bg-white text-gray-900 placeholder-gray-400"
            />
          </div>

          {/* Dynamic Extra Fields (Controlled by Settings) */}
          {hasExtraFields && (
              ...
              {settings.extractItemCount && (
                 <div className="space-y-1">
                  ...
                  <input 
                    ...
                    className="w-full px-3 py-2 rounded-lg border border-gray-200 text-sm bg-white text-gray-900 placeholder-gray-400 focus:ring-2 focus:ring-indigo-500" 
                  />
                </div>
              )}
              ...
              {settings.extractCategory && (
                 <div className="space-y-2">
                  ...
                  <input 
                    ...
                    onChange={handleChange} 
                    className="w-full px-3 py-2 rounded-lg border border-gray-200 text-sm bg-white text-gray-900 placeholder-gray-400 focus:ring-2 focus:ring-indigo-500" 
                    placeholder={t.placeholders.cat}
                  />
                  {/* Category Chips for Easy Selection */}
          ...
          {settings.extractNotes && (
             <div className="space-y-1">
              ...
              <textarea
                ...
                className="w-full px-3 py-2 rounded-lg border border-gray-300 focus:ring-2 focus:ring-indigo-500 bg-white text-gray-900 placeholder-gray-400 text-sm min-h-[80px]"
              />
            </div>
          )}
                 ...
                 {formData.items?.map((item, idx) => (
                   <div key={idx} className="flex gap-2">
                     <input 
                        ...
                        className="flex-1 px-2 py-1.5 text-sm rounded border border-gray-300 bg-white text-gray-900 placeholder-gray-400 focus:ring-1 focus:ring-indigo-500"
                     ...
                     <input 
                        ...
                        className="w-20 px-2 py-1.5 text-sm rounded border border-gray-300 text-right bg-white text-gray-900 placeholder-gray-400 focus:ring-1 focus:ring-indigo-500"
                        placeholder={t.placeholders.itemPrice}
                     />
      ...
      <div className="p-4 bg-white border-t border-gray-100">
        <button
          ...
          className="w-full py-4 bg-indigo-600 text-white rounded-xl font-bold text-lg shadow-lg hover:bg-indigo-700 transition active:scale-95"
        >
