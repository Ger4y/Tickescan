  ...
  return (
    <div className="min-h-screen w-full bg-gradient-to-br from-indigo-600 to-purple-700 flex flex-col items-center justify-start pt-4 sm:pt-10 p-4">
      <div className="w-full max-w-md bg-white rounded-3xl shadow-2xl overflow-hidden min-h-[600px] flex flex-col relative mb-4">
          ...
          <div className="flex items-center gap-0.5 shrink-0">
            ...
            <div className="relative mr-0.5" ref={langMenuRef}>
              <button 
                ...
                className="p-2 rounded-full hover:bg-white/20 transition-colors focus:outline-none text-lg"
                ...
                <div className="absolute top-full right-0 mt-2 bg-white rounded-xl shadow-xl py-2 min-w-[50px] overflow-hidden z-50 border border-gray-100 animate-fade-in">
                  {(Object.keys(FLAGS) as Language[]).map((lang) => (
                    <button
                      ...
                      className={`w-full text-center py-2 hover:bg-gray-100 text-xl ${language === lang ? 'bg-indigo-50' : ''}`}
                    >
                      {FLAGS[lang]}
                    </button>
                  ))}
        ...
        <main className="flex-1 flex flex-col overflow-hidden bg-gray-50 h-full relative">
          {children}
        </main>
        
        {/* Footer Credit */}
        <footer className="bg-gray-50 p-2 text-center border-t border-gray-100">
           <p className="text-[10px] text-gray-400 font-medium">App creada por Geray Padilla</p>
        </footer>
      </div>
    </div>
  );
